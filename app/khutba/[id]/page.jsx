...
export default async function KhutbaPage({ params }) {
  const k = await getKhutba(params.id)
  if (!k) notFound()

  const cleanTxt = k.channel_id ? await getCleanTranscript(k.channel_id, k.video_id) : null
  ...
  const showSuggestedAsSecondary = k.catchy_title && k.suggested_title && k.suggested_title !== primaryTitle
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <header className="mb-10">
        ...
        {showSuggestedAsSecondary && (
          <p className="text-sm text-dim mb-6">{k.suggested_title}</p>
        )}

        {/* Tags */}
        {k.topic_tags?.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-6">
            {k.topic_tags.map(tag => (
              <span
                key={tag}
      ...
      <section className="mb-16">
        ...
        {cleanTxt ? (
          ...
          <div className="text-center py-12 text-dim">
            <p className="text-sm">Transcript not yet available for this khutba.</p>
          </div>
        )}
      </section>
