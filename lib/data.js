...
/**
 * The MCC Tucson channel. All general browsing surfaces (homepage, This Week,
 * khutba archive, library, lecture browse) are scoped to this channel only.
 * Sh. Ali Mashhour's content appears ONLY in its own Study Guide section.
 * Lecture DETAIL pages remain unscoped (see getAllVideosAllChannels) so
 * Study Guide links to his lectures resolve.
 */
const MCC_CHANNEL_ID = 'UCt-XeQTVRSETC9DceeC6nMw'
...
/** All Jumaa Khutbas (MCC only), newest first */
export async function getAllKhutbas() {
  const { videos } = await readIndex()
  return videos
    .filter(v =>
      v.channel_id === MCC_CHANNEL_ID &&
      v.time_slot === 'Jumaa Khutba' &&
      v.has_transcript
    )
...
/**
 * All MCC videos with a transcript, regardless of type (Fajr/Zuhr/Isha/Jumaa
 * Khutba). Used by general browsing views like "This Week". Scoped to the MCC
 * channel so other channels' content never mixes into browse surfaces.
 */
export async function getAllVideos() {
  const { videos } = await readIndex()
  return videos.filter(v => v.has_transcript && v.channel_id === MCC_CHANNEL_ID)
}

/**
 * All videos with a transcript across ALL channels. Only for surfaces that
 * legitimately need everything: lecture detail pages / static params (so
 * Study Guide links to Sh. Ali's lectures resolve) and channel-specific
 * study sections. NOT for browse views.
 */
export async function getAllVideosAllChannels() {
  const { videos } = await readIndex()
...
/** Single video by video_id — ANY channel, so detail pages resolve for all */
export async function getVideo(videoId) {
  const all = await getAllVideosAllChannels()
...
/**
 * All video_ids for static path generation — ALL channels, so lecture detail
 * pages exist for Sh. Ali's videos too (Study Guide links must resolve).
 */
...
//
// Organized by YouTube playlist (ground truth) rather than AI-assigned
// hadith_book tags. Part numbers are extracted from video titles for ordering.
// Each series has a clean slug for the URL, mapped to its YouTube playlist ID.
//
// Content type is "tafsir" or "seerah-fiqh" — used only for display labels,
// not for data filtering (which uses playlist_id exclusively).

export const SHEIKH_ALI_SERIES = [
  {
    slug:        'zadul-maad',
    playlistId:  'PLNJENs8fj3IBehKwfPUHi5sWn5VKcxJsL',
    displayName: 'Provisions for the Hereafter (Zadul-Ma\'ad)',
    arabicName:  'زاد المعاد',
    author:      'Ibn al-Qayyim al-Jawziyyah',
    type:        'Seerah & Fiqh',
    description: 'A classical text covering the prophetic biography alongside '
                 + 'legal rulings and spiritual lessons derived from each event. '
                 + 'Taught by Sh. Ali Mashhour in an ongoing series.',
    partPattern: /part\s*(\d+)/i,
  },
  {
    slug:        'ramadan-tafsir-2024',
    playlistId:  'PLNJENs8fj3IDd2GBEXLWNeHzq_CjrcGkP',
    displayName: 'Ramadan Daily Quran Tafsir 2024',
    arabicName:  'تفسير القرآن الكريم',
    author:      'Sh. Ali Mashhour',
    type:        'Tafsir',
    description: 'Daily Quran tafsir sessions delivered during Ramadan 1445 / 2024.',
    partPattern: /day\s*(\d+)|part\s*(\d+)/i,
  },
  {
    slug:        'ramadan-tafsir',
    playlistId:  'PLNJENs8fj3IA53OsvMknKsBlCnky5Y9F0',
    displayName: 'Ramadan Daily Quran Tafseer',
    arabicName:  'تفسير القرآن الكريم',
    author:      'Sh. Ali Mashhour',
    type:        'Tafsir',
    description: 'Daily Quran tafsir sessions from Ramadan (earlier series).',
    partPattern: /day\s*(\d+)|part\s*(\d+)/i,
  },
  {
    slug:        'tafseer-al-anbiya',
    playlistId:  'PLNJENs8fj3IAsPET6pHFK6cNepp41lCn8',
    displayName: 'Tafseer Surat Al-Anbiya',
    arabicName:  'تفسير سورة الأنبياء',
    author:      'Sh. Ali Mashhour',
    type:        'Tafsir',
    description: 'A detailed tafsir of Surat Al-Anbiya (The Prophets), the 21st '
                 + 'chapter of the Quran.',
    partPattern: /part\s*(\d+)|session\s*(\d+)/i,
  },
  {
    slug:        'an-ayah',
    playlistId:  'PLNJENs8fj3IANGBDOF6di8BJrddJHLoJy',
    displayName: 'An Ayah: A Deeper Understanding',
    arabicName:  null,
    author:      'Sh. Ali Mashhour',
    type:        'Tafsir',
    description: 'Short focused reflections on individual Quranic verses.',
    partPattern: /part\s*(\d+)/i,
  },
  {
    slug:        'gaining-what-you-keep',
    playlistId:  'PLNJENs8fj3IBOxN0oIuw2UWh6-jFskl0_',
    displayName: 'Gaining What You Keep & Keeping What You Gain',
    arabicName:  null,
    author:      'Sh. Ali Mashhour',
    type:        'Lecture',
    description: 'A lecture series on building and preserving righteous deeds.',
    partPattern: /part\s*(\d+)/i,
  },
  {
    slug:        'modern-superstitions',
    playlistId:  'PLNJENs8fj3IBi-zBdhFuae-zwM7IwWa-q',
    displayName: 'The Darkness of Modern Superstitions Under the Light of Islam',
    arabicName:  null,
    author:      'Sh. Ali Mashhour',
    type:        'Lecture',
    description: 'Examining contemporary superstitions and misconceptions through '
                 + 'the lens of Islamic guidance.',
    partPattern: /part\s*(\d+)/i,
  },
]

export function getSheikhAliSeriesConfig(slug) {
  return SHEIKH_ALI_SERIES.find(s => s.slug === slug) ?? null
}

export function getAllSheikhAliSeriesSlugs() {
  return SHEIKH_ALI_SERIES.map(s => s.slug)
}

/**
 * Extract a part/day number from a video title for ordering within a series.
 * Returns null if no number is found — those videos sort after numbered ones.
 */
function extractPartNumber(title, pattern) {
  if (!title || !pattern) return null
  const m = title.match(pattern)
  if (!m) return null
  // pattern may have multiple capture groups (day|part) — find the first non-null
  const num = m.slice(1).find(g => g != null)
  return num ? parseInt(num, 10) : null
}

/**
 * Videos for one Sh. Ali series, ordered by part number then by stream date.
 * Uses playlist_id for filtering — more reliable than AI-assigned hadith_book.
 */
export async function getSheikhAliSeriesVideos(slug) {
  const series = getSheikhAliSeriesConfig(slug)
  if (!series) return []
  const all = await getAllVideosAllChannels()
  const videos = all.filter(
    v => v.playlist_id === series.playlistId && v.has_transcript
  )
  return videos.sort((a, b) => {
    const pa = extractPartNumber(a.title, series.partPattern)
    const pb = extractPartNumber(b.title, series.partPattern)
    // numbered videos first, in part order
    if (pa != null && pb != null) return pa - pb
    if (pa != null) return -1
    if (pb != null) return 1
    // fall back to stream date
    return (a.post_date || '').localeCompare(b.post_date || '')
  })
}

/** All Sh. Ali series with their fetched video counts, for the landing page. */
export async function getSheikhAliSeriesWithCounts() {
  const all = await getAllVideosAllChannels()
  return SHEIKH_ALI_SERIES.map(series => ({
    ...series,
    videoCount: all.filter(
      v => v.playlist_id === series.playlistId && v.has_transcript
    ).length,
  }))
}
